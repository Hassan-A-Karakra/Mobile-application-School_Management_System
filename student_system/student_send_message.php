...
if (!isset($data['message']) || !isset($data['student_ids']) || !isset($data['teacher_id'])) {
    echo json_encode([
        'status' => 'error',
        'message' => 'Missing required fields (message, student_ids, or teacher_id)'
    ]);
    exit;
}

$message_content = $conn->real_escape_string($data['message']); 
...
if (!$stmt_teacher) {
    error_log("Prepare failed for fetching teacher name: " . $conn->error);
    echo json_encode([
        'status' => 'error',
        'message' => 'Database error: Failed to prepare teacher name fetch'
    ]);
    exit;
}
$stmt_teacher->bind_param("i", $teacher_id);
if (!$stmt_teacher->execute()) {
    error_log("Execute failed for fetching teacher name: " . $stmt_teacher->error);
    echo json_encode([
        'status' => 'error',
        'message' => 'Database error: Failed to execute teacher name fetch'
    ]);
    exit;
}
...
if (empty($teacher_name)) {
    echo json_encode([
        'status' => 'error',
        'message' => 'Teacher not found or name is empty'
    ]);
    exit;
}

$title_for_db = $conn->real_escape_string(trim($teacher_name));
...
try {
    ...
    foreach ($student_ids as $student_id) {
        ...
        $stmt->bind_param("ssi", $title_for_db, $message_content, $int_student_id);
        if (!$stmt->execute()) {
